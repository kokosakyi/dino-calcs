...
export function ChannelBeamDesign() {
  ...
  const [factoredMoment, setFactoredMoment] = useState<number>(100);
  const [factoredShear, setFactoredShear] = useState<number>(50);

  // UDL mode inputs (Mode 2)
  const [span, setSpan] = useState<number>(4000);
  ...
  const [deadLoad, setDeadLoad] = useState<number>(2);
