...
export function ChannelBeamDesign() {
  ...
  const [factoredMoment, setFactoredMoment] = useState<number | null>(100);
  const [factoredShear, setFactoredShear] = useState<number | null>(50);

  // UDL mode inputs (Mode 2)
  const [span, setSpan] = useState<number | null>(4000);
  ...
  const [deadLoad, setDeadLoad] = useState<number | null>(2);
  ...
  const results = useMemo(() => {
    ...
    return findOptimalChannelSection(
      cSections as CSection[],
      {
        factoredMoment: Mf || 0,
        factoredShear: Vf || 0,
        steelGrade,
  ...
  return (
    <div className="beam-design-page">
      <header className="page-header">
        <h1>C-Channel Beam Design</h1>
        <p>Select the most economical C-channel section based on factored loads per CSA S16-19</p>
      </header>
