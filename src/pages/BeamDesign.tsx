...
export function BeamDesign() {
  ...
  const [factoredMoment, setFactoredMoment] = useState<number | null>(500);
  const [factoredShear, setFactoredShear] = useState<number | null>(200);

  // UDL mode inputs (Mode 2)
  const [span, setSpan] = useState<number | null>(6000);
  const [udlULS, setUdlULS] = useState<number | null>(25);
  const [udlSLS, setUdlSLS] = useState<number | null>(18);
  const [deflectionLimit, setDeflectionLimit] = useState<240 | 300 | 360>(360);

  // NBC mode inputs (Mode 3)
  const [deadLoad, setDeadLoad] = useState<number | null>(5);
  const [liveLoad, setLiveLoad] = useState<number | null>(10);
