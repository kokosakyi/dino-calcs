...
import { SCapacitySummary } from '../components/SCapacitySummary';
import { 
  calculateSMomentResistance, 
  calculateSShearResistance, 
  calculateSTensileResistance,
  calculateSLateralTorsionalBuckling,
  checkSSectionClass 
} from '../utils/steelDesign';
import type { SSection, SteelGrade, LateralSupportType, LateralTorsionalBucklingResult } from '../types/steel';
import { STEEL_PROPERTIES } from '../types/steel';
import sSections from '../assets/S_section.json';

interface SCapacityResult {
  section: SSection;
  Mr: number;
  Vr: number;
  Tr: number;
...
export function SCapacity() {
  // Section selection
  const [selectedDesignation, setSelectedDesignation] = useState<string>('S310x74');
  ...
  const sectionOptions = useMemo(() => {
    return (sSections as SSection[]).map(section => ({
      id: section.Dsg,
      label: section.Dsg,
  ...
  const selectedSection = useMemo(() => {
    return (sSections as SSection[]).find(s => s.Dsg === selectedDesignation) || null;
  }, [selectedDesignation]);
  ...
  const capacityResult = useMemo((): SCapacityResult | null => {
    ...
    if (lateralSupport === 'unsupported' && unbracedLength > 0) {
      ...
      if (sectionClass.overallClass <= 2) {
        ...
        Mr = calculateSMomentResistance(selectedSection, Fy);
      }
    } else {
    ...
    const Vr = calculateSShearResistance(selectedSection, Fy);
    const Tr = calculateSTensileResistance(selectedSection, Fy);
  ...
  return (
    <div className="section-capacity-page">
      <header className="page-header">
        <h1>S-Section Capacity Check</h1>
        <p>Calculate member resistance values for S-sections (American Standard Beams) per CSA S16-19</p>
      </header>
